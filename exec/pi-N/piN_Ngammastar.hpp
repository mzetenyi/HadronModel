...
class piN_Ngammastar {
 public:
  ...
  MultiArray<double> partialExpectationValues(double costh,
...
template <class Observable>
MultiArray<double> piN_Ngammastar::partialExpectationValues(
    double costh, const Observable& OBS) const {
            ...
            for (halfint lap : {-_1, _0, _1}) {
              ...
              for (halfint la1 : {-half, half}) {
                for (halfint la2 : {-half, half}) {
                  rho_la_lap += HA({R, al}, {la1, la2, la}) *
                                    conj(HA({Rp, alp}, {la1, la2, lap})) +
                                HA({Rp, alp}, {la1, la2, la}) *
                                    conj(HA({R, al}, {la1, la2, lap}));
                }
              }
...
MultiArray<double> piN_Ngammastar::partialExpectationValues(
  ...
  for (double costh(startcosth + dcosth / 2.); costh < maxcosth;
       costh += dcosth) {
    HelicityAmplitudes HA = helicityAmplitudes(costh);
    for (auto R : channels) {
      for (auto al : EMchannels) {
...
template <class Observable>
udouble piN_Ngammastar::bareExpectationValue(double costh,
                                             const Observable& OBS) const {
...
template <class Observable>
udouble piN_Ngammastar::expectationValue(double costh,
                                         const Observable& OBS) const {
  int npol = 2;  // No. of pol. states in pi+N
  double fac = 1. / (32. * pi_ * s) * pout_abs / pin_abs * 1. /
               npol;  // factor for cross section of piN->Ngamma*
  return fac * bareExpectationValue(costh, OBS);
}

template <class Observable>
udouble piN_Ngammastar::inverseExpectationValue(double costh,
                                                const Observable& OBS) const {
  int npol = 4;  // No. of pol. states in gamma+N (real photons, no longi pol!)
  double fac = 1. / (32. * pi_ * s) * pin_abs / pout_abs * 1. /
               npol;  // factor for cross section of piN->Ngamma*
  return fac * bareExpectationValue(costh, OBS);
}
...
udouble piN_Ngammastar::bareExpectationValue(const Observable& OBS) const {
  ...
  r2 *= 4. / (expVal * expVal);
  //  cerr << "return from expectationValue: " << expVal << " (" << sqrt(r2) <<
  //  ")" << endl;
  return udouble(expVal, sqrt(r2));
}
