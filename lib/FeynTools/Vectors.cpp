...
bool isIndex3(uint i) { return (1 <= i and i <= 3); }

bool isIndex4(uint mu) { return (0 <= mu and mu <= 3); }
...
const ThreeVector& ThreeVector::operator+() const { return *this; }

ThreeVector ThreeVector::operator-() const { return (-1) * (*this); }
...
ThreeVector ThreeVector::operator*(const double& c) const {
...
ThreeVector operator*(const double& c, const ThreeVector& v) {
...
const ThreeVector ThreeVector::e[] = {
    ThreeVector(0, 0, 0), ThreeVector(1, 0, 0), ThreeVector(0, 1, 0),
    ThreeVector(0, 0, 1)};
...
ThreeTensor::ThreeTensor(double x11, double x12, double x13, double x21,
...
const ThreeTensor& ThreeTensor::operator+() const { return *this; }

ThreeTensor ThreeTensor::operator-() const { return (-1) * (*this); }

ThreeTensor ThreeTensor::operator+(const ThreeTensor& other) const {
  return ThreeTensor(x[1][1] + other.x[1][1], x[1][2] + other.x[1][2],
                     x[1][3] + other.x[1][3], x[2][1] + other.x[2][1],
                     x[2][2] + other.x[2][2], x[2][3] + other.x[2][3],
                     x[3][1] + other.x[3][1], x[3][2] + other.x[3][2],
                     x[3][3] + other.x[3][3]);
}
...
ThreeTensor ThreeTensor::operator*(const ThreeTensor& other) const {
  return ThreeTensor(x[1][1] * other.x[1][1] + x[1][2] * other.x[2][1] +
                         x[1][3] * other.x[3][1],
                     x[1][1] * other.x[1][2] + x[1][2] * other.x[2][2] +
                         x[1][3] * other.x[3][2],
                     x[1][1] * other.x[1][3] + x[1][2] * other.x[2][3] +
                         x[1][3] * other.x[3][3],
                     x[2][1] * other.x[1][1] + x[2][2] * other.x[2][1] +
                         x[2][3] * other.x[3][1],
                     x[2][1] * other.x[1][2] + x[2][2] * other.x[2][2] +
                         x[2][3] * other.x[3][2],
                     x[2][1] * other.x[1][3] + x[2][2] * other.x[2][3] +
                         x[2][3] * other.x[3][3],
                     x[3][1] * other.x[1][1] + x[3][2] * other.x[2][1] +
                         x[3][3] * other.x[3][1],
                     x[3][1] * other.x[1][2] + x[3][2] * other.x[2][2] +
                         x[3][3] * other.x[3][2],
                     x[3][1] * other.x[1][3] + x[3][2] * other.x[2][3] +
                         x[3][3] * other.x[3][3]);
}

ThreeVector ThreeTensor::operator*(const ThreeVector& v) const {
...
ThreeVector operator*(const ThreeVector& v, const ThreeTensor& t) {
  return ThreeVector(
      v.x[1] * t.x[1][1] + v.x[2] * t.x[2][1] + v.x[3] * t.x[3][1],
      v.x[1] * t.x[1][2] + v.x[2] * t.x[2][2] + v.x[3] * t.x[3][2],
      v.x[1] * t.x[1][3] + v.x[2] * t.x[2][3] + v.x[3] * t.x[3][3]);
}

ThreeTensor diad(const ThreeVector& v, const ThreeVector& w) {
...
ThreeTensor ThreeTensor::operator*(const double& c) const {
  return ThreeTensor(x[1][1] * c, x[1][2] * c, x[1][3] * c, x[2][1] * c,
                     x[2][2] * c, x[2][3] * c, x[3][1] * c, x[3][2] * c,
                     x[3][3] * c);
}
...
ThreeTensor operator*(const double& c, const ThreeTensor& t) {
  return ThreeTensor(c * t.x[1][1], c * t.x[1][2], c * t.x[1][3], c * t.x[2][1],
                     c * t.x[2][2], c * t.x[2][3], c * t.x[3][1], c * t.x[3][2],
                     c * t.x[3][3]);
}

bool ThreeTensor::operator==(const ThreeTensor& other) const {
  return ((x[1][1] == other.x[1][1]) && (x[1][2] == other.x[1][2]) &&
          (x[1][3] == other.x[1][3]) && (x[2][1] == other.x[2][1]) &&
          (x[2][2] == other.x[2][2]) && (x[2][3] == other.x[2][3]) &&
          (x[3][1] == other.x[3][1]) && (x[3][2] == other.x[3][2]) &&
          (x[3][3] == other.x[3][3]));
...
void ThreeTensor::output(ostream& out, uint fieldWidth) const {
  out << "\n/ " << setw(fieldWidth) << x[1][1] << "  " << setw(fieldWidth)
      << x[1][2] << "  " << setw(fieldWidth) << x[1][3] << " \\\n"
      << "| " << setw(fieldWidth) << x[2][1] << "  " << setw(fieldWidth)
      << x[2][2] << "  " << setw(fieldWidth) << x[2][3] << " |\n"
      << "\\ " << setw(fieldWidth) << x[3][1] << "  " << setw(fieldWidth)
      << x[3][2] << "  " << setw(fieldWidth) << x[3][3] << " /" << endl;
}

double ThreeTensor::trace() const { return x[1][1] + x[2][2] + x[3][3]; }
...
const FourVector& FourVector::operator+() const { return *this; }

FourVector FourVector::operator-() const { return (-1) * (*this); }

FourVector FourVector::operator+(const FourVector& other) const {
  return FourVector(x[0] + other.x[0], x[1] + other.x[1], x[2] + other.x[2],
                    x[3] + other.x[3]);
}
...
FourVector FourVector::operator*(const double& c) const {
...
FourVector operator*(const double& c, const FourVector& v) {
...
FourVector FourVector::reverse() const {
  return FourVector(x[0], -x[1], -x[2], -x[3]);
}
FourVector FourVector::reflect() const {
  return FourVector(x[0], -x[1], -x[2], -x[3]);
}
...
FourTensor FourTensor::createUpDown(double x00, double x01, double x02,
                                    double x03, double x10, double x11,
                                    double x12, double x13, double x20,
                                    double x21, double x22, double x23,
                                    double x30, double x31, double x32,
                                    double x33) {
  return FourTensor(x00, -x01, -x02, -x03, x10, -x11, -x12, -x13, x20, -x21,
                    -x22, -x23, x30, -x31, -x32, -x33);
}
...
FourTensor::FourTensor(double x00, const ThreeVector& x0j,
                       const ThreeVector& xi0, const ThreeTensor& xij) {
  x[0][0] = x00;
  for (uint i(1); i < 4; i++) {
    x[0][i] = x0j(i);
    x[i][0] = xi0(i);
...
FourVector FourTensor::operator()(uint mu) const {
  assert(isIndex4(mu));
  return FourVector(x[mu][0], x[mu][1], x[mu][2], x[mu][3]);
}

const FourTensor& FourTensor::operator+() const { return *this; }

FourTensor FourTensor::operator-() const { return (-1) * (*this); }

FourTensor FourTensor::operator+(const FourTensor& other) const {
  FourTensor ret;
...
FourVector FourTensor::operator*(const FourVector& v) const {
  FourVector ret;
  for (uint mu(0); mu < 4; mu++) {
    for (uint nu(0); nu < 4; nu++) {
...
FourVector operator*(const FourVector& v, const FourTensor& t) {
  FourVector ret;
  for (uint mu(0); mu < 4; mu++) {
    for (uint nu(0); nu < 4; nu++) {
...
FourTensor diad(const FourVector& v, const FourVector& w) {
  FourTensor ret;
  for (uint mu(0); mu < 4; mu++) {
    for (uint nu(0); nu < 4; nu++) {
...
FourTensor FourTensor::operator*(const double& c) const {
  FourTensor ret;
  for (uint mu(0); mu < 4; mu++) {
    for (uint nu(0); nu < 4; nu++) {
...
FourTensor operator*(const double& c, const FourTensor& t) {
  FourTensor ret;
  for (uint mu(0); mu < 4; mu++) {
    for (uint nu(0); nu < 4; nu++) {
...
LorentzTransformation::LorentzTransformation(const FourVector& a) : a(a) {
  ...
  if (msqr > 0) {  // a is timelike
    if (a.spacial() == ThreeVector::nullVector) {
      ...
      const ThreeTensor Lspacial =
          ThreeTensor::unitTensor + (a(0) / m - 1.) * diad(aa, aa) / (aa * aa);
      L = FourTensor(a(0) / m, aa / m, aa / m, Lspacial);
      /*
      const FourVector u = a/m;
      const ThreeVector uu = u.spacial();
      const double gamma = u[0];
      const ThreeTensor Lspacial = ThreeTensor::unitTensor +
      diad(uu,uu)/(gamma+1.); L = FourTensor(gamma,uu,-uu,-Lspacial);
      */
    }
    ...
    if (a(0) == 0) {
      ...
      const ThreeTensor Lspacial =
          ThreeTensor::unitTensor + (aaabs / m - 1.) * diad(aa, aa) / (aa * aa);
      L = FourTensor(aaabs / m, a(0) / (m * aaabs) * aa,
                     a(0) / (m * aaabs) * aa, Lspacial);
    }
  } else {
    /*
      cerr << "LorentzTransformation::constr() - a is light-like or NaN" <<
    endl; PR(a);
    //*/
    L = FourTensor::unitTensor;
  }
...
LorentzTransformation LorentzTransformation::fromRestFrame(
    const FourVector& a) {
