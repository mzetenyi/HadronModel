...
//                     ((not Config::exists("vector")) and (not Config::exists("tensor")))
//                     );
...
bool vectorRho() {
  static bool vRho(isSet("vector") or isSet("vectorRho") or (not (isSet("tensor") or isSet("tensorRho") or isSet("tensorGamma") or isSet("vectorGamma"))));
  return vRho;
}

bool tensorRho() {
  static bool tRho(isSet("tensor") or isSet("tensorRho") or (not (isSet("vector") or isSet("vectorRho") or isSet("vectorGamma") or isSet("tensorGamma"))));
  return tRho;
}

bool vectorGamma() {
  static bool vGamma(isSet("vector") or isSet("vectorGamma") or (not (isSet("tensor") or isSet("tensorGamma") or isSet("tensorRho") or isSet("vectorRho"))));
  return vGamma;
}

bool tensorGamma() {
  static bool tGamma(isSet("tensor") or isSet("tensorGamma") or (not (isSet("vector") or isSet("vectorGamma") or isSet("vectorRho") or isSet("tensorRho"))));
  return tGamma;
}

bool vectorOmega() {
  static bool vOmega(isSet("vector") or isSet("vectorOmega") or (not (isSet("tensor") or isSet("tensorOmega"))));
  return vOmega;
}

bool tensorOmega() {
  static bool tOmega(isSet("tensor") or isSet("tensorOmega") or (not (isSet("vector") or isSet("vectorOmega"))));
  return tOmega;
}
...
DiracMatrix vertexNNgamma(uint mu, int Qin, FourVector k) {
  ...
  if (tTerm) {
    //    ret += -i_*e * kappa/(4.*mn)*(gamma_(mu)*gamma_(k) - gamma_(k)*gamma_(mu));  // original - sign mistake c.f. Ericson-Weise
    ret += i_*e * kappa/(4.*mn)*(gamma_(mu)*gamma_(k) - gamma_(k)*gamma_(mu));
  }
  return ret;
}
...
DiracMatrix vertexNNrho(uint mu, FourVector k, int Qin, int Qmes) {
  ...
  DiracMatrix ret = gamma_null;
  if (vTerm) {
    ret += i_*grho_tilde/2. * iso1hh(Qin,Qmes) * gamma_(mu); // only vector term
  }
  if (tTerm) {
    ret += i_*grho_tilde/2. * iso1hh(Qin,Qmes) * kappa/(4.*mn)*(gamma_(mu)*gamma_(k) - gamma_(k)*gamma_(mu)); // only tensor term
  }
  return ret;
}
...
DiracMatrix vertexNNomega(uint mu, FourVector k) {
  ...
  if (tTerm) {
    ret += i_*g_NNomega/2. * kappa/(4.*mn)*(gamma_(mu)*gamma_(k) - gamma_(k)*gamma_(mu));
  }
  return ret;
}
